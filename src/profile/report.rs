...
fn load_capture(input: &Path) -> Result<CaptureMetadata> {
    let payload = read_limited(&input.join(CAPTURE_FILE), CAPTURE_LIMIT_BYTES)?;
    let capture = serde_json::from_str::<CaptureMetadata>(&payload)
